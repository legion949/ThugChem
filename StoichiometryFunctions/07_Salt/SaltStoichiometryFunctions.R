...
# Resolution 07: Salt
SaltEquation <- function(input_atomic_number1 = NULL, input_valence1 = NULL,
  ...
  ####################################################
      ...
      # Part 1: Structure of the "matrix_solution"
      {
        ...
        # 1.0) Explanation about Part 1 (if you want to read)
        {
          ...
          # Each row is a step on the stoichiometry resolution (21 steps for Salt = 21 rows).
          ...
          # The Hydride
          #  7) coefficient of the hydride in reactants
          #  8) Abbreviation of the Hydrogen in the hydride in reagents
          #  9) Sub-index of the Hydrogen in the hydride in reactants
          # 10) Abbreviation of the Non-metal element the hydride in reagents
          # 11) Sub-index of the Non-metal element the hydride in reagents
          
          # The Salt
          # 12) coefficient of the salt in products
          # 13) Abbreviation of the Metal Element in the salt in products
          # 14) Sub-index of the Metal Element in the salt in products
          # 15) Abbreviation of the Non-metal in the salt in products
          # 16) Sub-index of the Non-metal in the salt in products          
        ...
        # 1.1) The creation for "matrix_solution"
        {
          ...
          # Total Steps for Salt Resolution (TSFSR)
          TSFSR <- 9
          
          
          my_names <- c("Order", 
                        "Coef1", "E01", "Sub1","E02", "Sub2",    
                        "Coef2", "E03", "Sub3","E04", "Sub4", 
                        "Coef3", "E05", "Sub5", "E06", "Sub6", 
                        "Coef4", "E07", "Sub7", "E08", "Sub8")
          
          
          matrix_solution <- data.frame(matrix(NA, TSFSR, length(my_names)))
      ...
      # Part 2: Stoichiometry Resolution for Salt
      {
        ###
        ...
        if (internal_control == 1) {
          ...
          # 2.1) New Details
          {
            ...
            if (dt5_out == TRUE) {
              ...
              example2 <- HydrideEquation(input_atomic_number1 = atomic_number2,
            ...
            if (dt6_out == TRUE) {
              ...
              example2 <- HydrideEquation(input_atomic_number1 = atomic_number1,
                                         input_valence1 = valence1,
                                         input_internal_language = "en",
                                         input_external_language = input_external_language,
                                         input_PeriodicTable = input_PeriodicTable)
            ...
            # Hydrure chemical formule
            cf2_element1 <- as.character(example2[nrow(example2),  9])
            cf2_sub1 <- example2[nrow(example2), 10]
            cf2_element2 <- as.character(example2[nrow(example2), 11])
            cf2_sub2 <- example2[nrow(example2), 12]
      ...
      } # End Part 2: Stoichiometry Resolution for Salt
      ############################################################
      ...
      # Parte3: Special cases
      {
        ...
        if (internal_control == 2) {
          ...
          matrix_solution <- data.frame(matrix("Salt Problems", nrow(matrix_solution), ncol(matrix_solution)))
        } # 
        ###  
...
InternalControl_Salt <- function(input_atomic_number1 = NULL, input_valence1 = NULL,
                                    input_atomic_number2 = NULL, input_valence2 = NULL,
                                    input_internal_language = "en", input_external_language = NULL,
                                    input_PeriodicTable = NULL) {
