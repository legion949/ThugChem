...
# LaTeX of Hydroxide
HydroxideLaTeX <-function(input_atomic_number1 = NULL, input_valence1 = NULL, 
  ...
  # Part 1: General structure of "matrix_latex"
  {
    ...
    matrix_latex <- matrix(NA, nrow(input_hydroxide_resolution), length(my_names))
  ...
  { 
    ...
    total_step <- nrow(input_hydroxide_resolution)
    
    
    order_step <- c(1:nrow(matrix_latex))
    
    coef1 <- input_hydroxide_resolution[,2]
    armed_oxide <- paste0(input_hydroxide_resolution[,3], "[",
                          input_hydroxide_resolution[,4], "]","*",
                          input_hydroxide_resolution[,5], "[",
                          input_hydroxide_resolution[,6], "]") 
    
    sign1 <- rep("'+'", total_step)
    
    coef2 <- input_hydroxide_resolution[,7]
    armed_water <- paste0(input_hydroxide_resolution[, 8], "[",
                          input_hydroxide_resolution[, 9], "]","*",
                          input_hydroxide_resolution[,10], "[",
                          input_hydroxide_resolution[,11], "]") 
    
    coef3 <- input_hydroxide_resolution[,12]
    armed_hydroxide <- paste0(input_hydroxide_resolution[,13], "[",
                                             input_hydroxide_resolution[,14], "]","*",
                                             input_hydroxide_resolution[,15], "[",
                                             input_hydroxide_resolution[,16], "]")  
    ...
    matrix_latex[,8] <- armed_hydroxide
...
# 
# 
# latex_hydroxide_resolution   <-         HydroxideLaTeX(input_atomic_number1 = input_atomic_number1,
#                                                        input_valence1 = input_valence1,
#                                                        input_internal_language = "en",
#                                                        input_external_language = input_external_language,
#                                                        input_PeriodicTable = input_PeriodicTable)
