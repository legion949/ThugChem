...
# LaTeX of Oxide
OxideLaTeX <-function(input_atomic_number1 = NULL, input_valence1 = NULL, 
  ...
  ####################################################
      ...
      # Part 1: General structure of "matrix_latex"
      {
        ###
          ...
          matrix_latex <- matrix(NA, nrow(input_oxide_resolution), length(my_names))
      ...
      { 
        ...
        total_step <- nrow(input_oxide_resolution)
        

        order_step <- c(1:nrow(matrix_latex))
        
        coef1 <- input_oxide_resolution[,2]
        armed_element <- paste0(input_oxide_resolution[,3], "[",input_oxide_resolution[,4], "]") 
        
        sign1 <- rep("'+'", total_step)
        
        coef2 <- input_oxide_resolution[,5]
        armed_oxygen <- paste0(input_oxide_resolution[,6], "[",input_oxide_resolution[,7], "]") 

        coef3 <- input_oxide_resolution[,8]
        armed_oxide <- paste0(input_oxide_resolution[,9], "[",input_oxide_resolution[,10], "]","*",
                              input_oxide_resolution[,11], "[",input_oxide_resolution[,12], "]") 
        ...
        matrix_latex[,6] <- armed_oxygen
        matrix_latex[,7] <- coef3
        matrix_latex[,8] <- armed_oxide
  ...
  ###########################################################################
      ...
      {
      ###
        # Normal case and Oxygen case don't need changes.
        # But, for Noble gas we must make a new column.
        if (internal_control == 3) { 
          
          # Columns separation
          my_new <- matrix(unlist(strsplit(matrix_latex[,"Oxide"], "\\*")),nrow(matrix_latex) , 2, byrow = T)
          colnames(my_new) <- c("NobleGas", "OxygenProd")
          ...
          # Empty values for more phamtons
          matrix_latex_v2[c(1,2),c(7,8,11)] <- paste0("phantom(", matrix_latex_v2[c(1,2),c(7,8,11)], ")")
...
# 
# 
# latex_oxide_resolution   <-         OxideLaTeX(input_atomic_number1 = input_atomic_number1,
#                                         input_valence1 = input_valence1,
#                                         input_internal_language = "en",
#                                         input_external_language = input_external_language,
#                                         input_PeriodicTable = input_PeriodicTable)
